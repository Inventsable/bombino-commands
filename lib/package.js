...
module.exports = async function () {
  ...
  if (ANSWERS.runSign) {
    let response = await promptUserOfPassword();
  ...
  if (targetZXP.length > 1) {
    targetZXP = targetZXP
    .sort((a, b) => {
      return +b.replace(/.*_/, '').replace(/\.zxp$/, '').split('.').join("") - +a.replace(/.*_/, '').replace(/\.zxp$/, '').split('.').join("")
    })[0]
  ...
  console.log("")
  spinner = ora({
      text: `Bundling ${chalk.green(`./archive/${targetZXP}`)} and ${chalk.green(`./package/`)}...`,
      spinner: utils.ORA_SPINNER
  }).start();
  ...
  spinner = ora({
    ...
    zipDirectory(input, output).then(() => {
      ...
      console.log("")
      spinner = ora({
            text: `Removing ${chalk.green(`./archive/${packageName}`)} staging folder...`,
            spinner: utils.ORA_SPINNER
      }).start();
...
async function stagePackageFolder(packageName, ZXPNewName, ZXPName) {
  ...
  try {
    ...
    await fse.copy(`./archive/${ZXPName}`, `./archive/${packageName}/${ZXPNewName}.zxp`);
...
async function readDir(targetPath) {
  return new Promise((resolve, reject) => {
    ...
    fs.readdir(path.resolve(targetPath), { encoding: "utf-8" }, (err, files) => {
      if (err) reject(err);
      resolve(files);
    });
  });
